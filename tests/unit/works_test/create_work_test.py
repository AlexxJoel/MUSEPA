...
from modules.works.create_work.app import lambda_handler
from modules.works.create_work.app import validate_connection, validate_event_body, validate_payload
from modules.works.create_work.connect_db import get_db_connection,get_secrets
from modules.works.create_work.authorization import authorizate_user
...
class TestCreateEvent(TestCase):
    ...
    @patch("modules.works.create_work.app.get_db_connection")
    @patch("modules.works.create_work.app.authorizate_user")
    ...
    def test_create_work_success(self, mock_validate_payload, mock_validate_event_body, mock_validate_connection,
        ...
        self.assertEqual(result["body"], json.dumps({"message": "Work created successfully"}))
...
class TestConnectDB(TestCase):
    @patch('modules.works.create_work.connect_db.psycopg2.connect')
    @patch('modules.works.create_work.connect_db.get_secrets')
