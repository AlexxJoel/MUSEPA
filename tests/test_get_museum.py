...
from crud_museums.get_Museum import lambda_handler, get_museums
...
def mock_cursor():
    cursor = MagicMock()
    cursor.fetchall.return_value = [{"id": 1, "name": "Test Museums"}]
    return cursor
...
def test_lambda_handler_success(mock_conn, mock_cursor):
    ...
    assert body['message'] == "Museums retrieved successfully."
