...
def validate_payload(payload):
    ...
    if "phone_number" not in payload or not isinstance(payload["phone_number"], str) or not phoneNumber_regex.match(payload["phone_number"]):
        return {"statusCode": 400, "body": json.dumps({"error": "Invalid or missing 'phone_number'"})}

    if "address" not in payload or not isinstance(payload["address"], str) or not letters_regex.match(payload["address"]):
        return {"statusCode": 400, "body": json.dumps({"error": "Invalid or missing 'address'"})}

    if "birthdate" not in payload or not isinstance(payload["birthdate"], str) or not date_regex.match(payload["birthdate"]):
